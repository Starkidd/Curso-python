...
from contabilidad import MiVentana

class CursoPythonApp(Gtk.Application):
	"""docstring for ClassName"""
	def __init__(self, *args,**kwargs):
		super(CursoPythonApp, self).__init__(*args,application_id = 'ni.edu.udem.cursopython.kg',**kwargs)
		self.window = None


	def do_activate(self):
		if not self.window:
			self.window = MiVentana(application = self, title = 'contabilidad')
